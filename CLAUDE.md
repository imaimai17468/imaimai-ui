...
|-----------|-----------|-------------|------|
| **新機能追加** | Phase 1-12 全て | 60-120分 | 完全なワークフロー |
| **中規模バグ修正** | 1,4,5,6,8,9A,10,11,12 | 30-60分 | 調査→実装→テスト→確認 |
| **UI/デザイン調整** | 1,3,4,5,8,9A,10,11,12 | 20-40分 | UIデザインレビュー含む |
| **小規模リファクタ** | 1,4,5,8,10,11,12 | 15-30分 | 既存パターン踏襲 |
| **タイポ修正** | 5,8,10,11,12 | 5分 | 設定ファイルや小さな修正 |
| **ドキュメント更新** | 5,10,11,12 | 5-10分 | ドキュメントのみの変更 |
...
10. **Phase 10: User Confirmation** - ユーザー承認を取得
11. **Phase 11: Git Commit** - コミット作成
12. **Phase 12: Push** - リモートへプッシュ
...
### Phase 10: User Confirmation (ユーザー確認) 【必須】

**重要**: コミットとプッシュの前に必ずユーザーの承認を得る
...
#### 2. ユーザーへの確認
- **必ず `AskUserQuestion` ツールを使用**してユーザーに確認を求める
- 「コミットしてプッシュしてもよろしいですか？」と明示的に質問
- ユーザーが承認するまで次のフェーズに進まない

#### 3. フィードバックへの対応
- ユーザーから修正要望があれば Phase 5 に戻って対応
- ユーザーが追加の確認を求めた場合は適切に対応

**重要な注意事項:**
- **絶対にユーザーの承認なしでコミット・プッシュしない**
- ユーザーが明示的に「OK」「はい」「承認」などと返答するまで待つ
- 自動的にコミットやプッシュを実行してはいけない

**完了チェックリスト:**
- [ ] 変更内容をユーザーに報告
- [ ] AskUserQuestionでユーザーの承認を取得
- [ ] ユーザーが承認を与えた

---

### Phase 11: Git Commit 【必須】
...
### Phase 12: Push 【Phase 11の後にのみ実行】

**使用ツール**: Bash tool

**重要**: このフェーズは Phase 11 (Git Commit) が完了した後にのみ実行する
